...
public class ClusterNodeEntry {
    ...
    // REVIEW: as far as I can see this method is only used for tests? If so, then maybe move it over to src/test/java
    // in a cluster node util class, rather than leaving it here in production code.
    @Nullable
    ...
    // REVIEW: seems a bit of a heavy-weight approach to toString - maybe just use the one generated by IntelliJ? Or
    // is there a particular reason you need this to be JSON? If so, then a dedicated toJson method would likely be
    // better.
    @Override
